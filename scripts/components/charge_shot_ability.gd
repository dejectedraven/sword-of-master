...
func get_charge_ratio() -> float:
	return min(_charge_time / max(GameConfig.archer_charge_max_time, 0.001), 1.0)
...
func release_charge() -> Dictionary:
	...
	return {
		"dir": dir, "damage": damage, "speed": speed, "range": range,
		"ratio": ratio
	}

func fire_arrow(dir: Vector2, damage: float, speed: float, range: float):
