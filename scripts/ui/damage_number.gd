...
static func spawn(parent: Node, world_pos: Vector2, amount: float, color: Color = Color.WHITE):
	spawn_label(parent, world_pos, str(int(round(amount))), color)

static func spawn_label(parent: Node, world_pos: Vector2, text: String, color: Color = Color.WHITE):
	if not is_instance_valid(parent): return
	var n = DamageNumber.new()
	n.text = text
	n.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	n.size = Vector2(120, 24)
	n.position = world_pos + Vector2(randf_range(-12, 12) - 60, -56)
	n.z_index = 100
