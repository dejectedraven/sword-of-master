...
### ✨ 新增
- **黑夜与视野**：CanvasModulate 夜色 + 角色视野光圈（程序生成径向渐变，无需素材）+ 相机缩放（默认 1.0 原始比例，GameConfig 可调）
...
- **道具系统**：宝箱 2→5 个；掉落**金币/道具互斥**（金币概率 50% 可调，道具用于稀释金币产出，逼迫多开箱）
...
### 🔧 修复
- **阴影遮挡角色**：新增角色自照小光（不投影，树影下也可见）；树/灌木在玩家躲到其后时淡出至 35%（可透过树冠看到角色）
